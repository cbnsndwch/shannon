...
export function validateName(name: string): ValidationResult {
  ...
  if (name.startsWith(".")) {
    return { ok: false, error: `invalid profile name '${name}': must not start with '.'` };
  }
  if (name.includes("..")) {
    return { ok: false, error: `invalid profile name '${name}': must not contain '..'` };
  }
  if (name.includes("/")) {
    return { ok: false, error: `invalid profile name '${name}': must not contain '/'` };
  }
  ...
  if (!VALID_NAME.test(name)) {
    return { ok: false, error: `invalid profile name '${name}': use only letters, digits, hyphens, underscores` };
  }
  return { ok: true };
}
...
export function assertValidName(name: string): void {
  ...
  if (!result.ok) {
    throw new ShannonError(result.error ?? `invalid profile name '${name}'`);
  }
}
