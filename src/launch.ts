 ...
 */
export function launchClaude(args: string[], env: NodeJS.ProcessEnv = process.env): number {
  ...
  if (!claude) {
    throw new ShannonError("'claude' binary not found in PATH. Is Claude Code installed?");
  }
  ...
  if (result.error) {
    throw new ShannonError(`failed to launch claude: ${result.error.message}`);
  }
